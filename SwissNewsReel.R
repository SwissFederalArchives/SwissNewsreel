...
# Functions ----
makerequest <- function(skip,take) {
  print(paste("requesting records",skip,":",skip+take,"from",totalpages))
  jsonrequest <- paste0('{
    "query":{
      "searchGroups":[
        {"searchFields":[
          {"value":"J2.143#1996/386*","key":"referenceCode"}
        ],"fieldOperator":1}
      ],
      "groupOperator":1
    },
...
getDescription <- function(id) {
  print(paste("fetching record",id,"(",counter,"/",total,")"))
  counter <<- counter + 1
  resget <- GET(
...
# Send a first request to find the total number of pages  ----
url <- "https://www.recherche.bar.admin.ch/recherche/api/v1/entities/Search"
res <- makerequest(0,1)
status_code(res)
headers(res)
fws <- content(res)
totalpages <- fws$entities$paging$total

# Fetch all data by bunches of 100 ----
stepstarts <- seq(1, ceiling(totalpages/100)*100, by = 100)
fws.datatable <- lapply(stepstarts,function(i){
  res <- makerequest(i,100)
  fws <- content(res)
  data.table(
    ...
    level = sapply(fws$entities$items,function(x) return(x$level)),
...
counter <- 1
total <- nrow(fws.datatable)
fws.datatable[, darin := lapply(refCode, function(x) {x = getDescription(x); x[["withinInfo"]] %>% unlist})]
...
fws.datatable$level <- unlistColumn(fws.datatable$level)
...
emissions_par_month <- emissions_par_date[, .N, by=.(month,year)]
ggplot(emissions_par_month) +
  geom_col(aes(x=month,y=N),colour="white", fill="darkred") + 
  scale_x_continuous(
    breaks=c(3,6,9,12),
